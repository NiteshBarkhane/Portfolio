                ...
                {items.map(item => (
                    <div key={item._id} className="glass-card p-4 flex justify-between items-center border border-white/5 hover:border-accent/50 transition-all">
                        ...
                        <div className="flex gap-2">
                            <button onClick={() => { setFormData(item); setIsEditing(true); }} className="p-2 hover:bg-white/5 rounded text-white hover:text-accent"><Edit2 size={16} /></button>
                            <button onClick={() => handleDelete(item._id)} className="p-2 hover:bg-white/5 rounded text-red-400 hover:text-red-500"><Trash2 size={16} /></button>
                        </div>
                    </div>
                ))}
            </div>
